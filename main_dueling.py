...
def optimize_model(representAgent: BisimAgent):
    ...
    next_state_values[non_final_mask] = target_net(non_final_next_states_represented).max(1)[0].detach()
...
def train(env, n_episodes, representAgent:BisimAgent ,render=False):
    for episode in range(n_episodes):
        ...
        if episode % (n_episodes // 3) == 0:
            torch.save(policy_net, "./results/models/duel_{}_model".format(episode // (n_episodes // 3)))
            representAgent.save("./results/bisim_models/duel_{}_model".format(episode // (n_episodes // 3)))
    env.close()
    return
...
def plot(avg):
    ...
    fig.plot(avg)
    fig.set_title('dueling-DQN Average Reward Cuves')
    fig.set(xlabel = 'Episodes',ylabel = 'Avg Reward')
    plt.savefig('./results/images/duel_curves.png')
...
if __name__ == '__main__':
    ...
    np.save('./results/arrays/duel_avg.npy', avg)
    plot(avg)
    torch.save(policy_net, "./results/models/duel_final_model")
    bisim.save("./results/bisim_models/duel_final_model")

    policy_net = torch.load("./results/models/duel_0_model")
    bisim.load("./results/bisim_models/duel_0_model")
    test(env, 1, policy_net, bisim, path='./videos/' + 'duel_video_0', render=False, rec=False)

    policy_net = torch.load("./results/models/duel_1_model")
    bisim.load("./results/bisim_models/duel_1_model")
    test(env, 1, policy_net, bisim, path='./videos/' + 'duel_video_1', render=False, rec=False)

    policy_net = torch.load("./results/models/duel_2_model")
    bisim.load("./results/bisim_models/duel_2_model")
    test(env, 1, policy_net, bisim, path='./videos/' + 'duel_video_2', render=False, rec=False)

    policy_net = torch.load("./results/models/duel_final_model")
    bisim.load("./results/bisim_models/duel_final_model")
    test(env, 1, policy_net, bisim, path='./videos/' + 'duel_video_3', render=False, rec=False)
