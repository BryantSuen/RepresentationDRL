...
def train(env, n_episodes, representAgent:BisimAgent, render=False):
    for episode in range(n_episodes):
        ...
        if episode % (n_episodes//3) == 0:
            torch.save(policy_net, "./results/models/dqn_{}_model".format(episode//(n_episodes//3)))
            representAgent.save("./results/bisim_models/dqn_{}_model".format(episode//(n_episodes//3)))
...
def plot(avg):
    ...
    fig.plot(avg)
    fig.set_title('DQN Average Reward Cuves')
    fig.set(xlabel = 'Episodes',ylabel = 'Avg Reward')
    plt.savefig('./results/images/DQN_curves.png')
...
if __name__ == '__main__':
    ...
    np.save('./results/arrays/DQN_avg.npy',avg)
    plot(avg)
    torch.save(policy_net, "./results/models/dqn_final_model")
    bisim.save("./results/bisim_models/dqn_final_model")

    policy_net = torch.load("./results/models/dqn_0_model")
    bisim.load("./results/bisim_models/dqn_0_model")
    test(env, 1, policy_net, bisim, path='./videos/' + 'dqn_video_0',render=False,rec=False)

    policy_net = torch.load("./results/models/dqn_1_model")
    bisim.load("./results/bisim_models/dqn_1_model")
    test(env, 1, policy_net, bisim, path='./videos/' + 'dqn_video_1', render=False, rec=False)

    policy_net = torch.load("./results/models/dqn_2_model")
    bisim.load("./results/bisim_models/dqn_2_model")
    test(env, 1, policy_net, bisim, path='./videos/' + 'dqn_video_2', render=False, rec=False)

    policy_net = torch.load("./results/models/dqn_final_model")
    bisim.load("./results/bisim_models/dqn_final_model")
    test(env, 1, policy_net, bisim, path='./videos/' + 'dqn_video_3', render=False, rec=False)
